...
- 创作者页面按固定 Schema 展示热门素材、规则预设和结构化成品；Prompt/模型编排不进入日常操作路径。
- 内部 Prompt 变量 Schema 校验、工作区权限、CSRF、幂等运行。
...
1. 在 `/generate` 选择按播放量排序的热门作品、按热度排序的新闻/事件，或填写自定义事实材料。
2. 选择已发布的规则预设；可选调整成片长度、受保护答案词和补充创作重点。
3. 点击“立即生成内容包”。系统自动使用工作区已配置的真实 LLM；没有真实配置时只允许显式 Mock 测试结果。
4. 在结果页直接查看英文 TTS、中文翻译、双语标题、搜索词、素材词、标签、事实来源和 QA。Prompt、模型和十步状态只在“审计与技术信息”中展示。

`/prompts` 和 `/workflows` 的后端能力继续保留给版本审计和维护，但不进入主导航，创作者无需理解或选择它们。
...
- 内部 Prompt/API 调试响应递归脱敏 key/token/secret/password/cookie/authorization 字段。
