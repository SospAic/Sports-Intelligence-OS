...
> 设置中心增量以 0009 落地 `llm_provider_settings`：工作区/Provider 唯一，连接配置只存 Fernet 密文和脱敏摘要，默认模型、采样参数、输入/输出费率、启停与健康状态结构化保存。生成运行仍冻结本次实际模型和参数，不随后续设置修改而变化。

> 安全增量以 0010 落地 `login_attempts`：只保存 HMAC 身份哈希、HMAC 客户端地址哈希、尝试时间和成功状态；身份/时间、IP/时间建复合索引。每小时维护任务删除超过保留期的尝试与失效 Session，不保存提交邮箱、原始 IP 或密码。
