...
def main() -> None:
    ...
    parser.add_argument("compose_file", type=Path, nargs="?", default=Path("docker-compose.yml"))
    ...
    for service_name in ("worker", "beat"):
        service = require_mapping(services[service_name], service_name)
        if "api" not in require_mapping(service.get("depends_on"), f"{service_name}.depends_on"):
            raise SystemExit(f"{service_name} must wait for the API readiness gate")

    if "SIO_BOOTSTRAP_ADMIN_PASSWORD: ${SIO_BOOTSTRAP_ADMIN_PASSWORD:-}" not in compose_text:
        raise SystemExit("bootstrap administrator password must default to an empty value")
    env_example_lines = {
        line.strip() for line in Path(".env.example").read_text(encoding="utf-8").splitlines()
    }
    ...
    if "API_INTERNAL_URL=http://api:8000" not in web_dockerfile:
        raise SystemExit("Web image must bake the Compose API hostname into Next.js rewrites")
    ...
    print("Static Compose validation passed for services: " + ", ".join(sorted(services)))
